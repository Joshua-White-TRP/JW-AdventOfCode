...
def vertical_search(box_positions, d):
    above_positions = [[x[0] + d[0], x[1] + d[1]] for x in box_positions]
    above_positions = [[x[0], x[1]] for x in above_positions if map[x[0]][x[1]] != '.']
    if len(above_positions) == 0:
        return []
    
    symbols = [map[x[0]][x[1]] for x in above_positions]
    if '#' in symbols:
        return False
    
    for a in above_positions:
        new_value = [a[0], a[1] + (1 if map[a[0]][a[1]] == '[' else - 1)]
...
for d in directions:
    ...
    if n_symbol == '.':
        ...
        res = horizontal_search(r_pos, d) if d in [ds['>'], ds['<']] else vertical_search([r_pos], d)
        if res:
            for r in res[::-1]:
                map[r[0] + d[0]][r[1] + d[1]] = map[r[0]][r[1]]
                map[r[0]][r[1]] = '.'
            map[r_pos[0]][r_pos[1]] = '.'
            map[n_pos[0]][n_pos[1]] = '@'
            r_pos = n_pos
