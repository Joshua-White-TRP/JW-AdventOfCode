...
def move(p, vis, dsf_idx, score):
    ...
    loc = (p, dsf_idx)
    if score > ans or (loc in unique_locations and unique_locations[loc] < score):
        return
...
unionised_set = ans_paths[0]
print(ans_paths)
for path in ans_paths[1:]:
    unionised_set.update(path)
print(ans)
print(len(unionised_set))