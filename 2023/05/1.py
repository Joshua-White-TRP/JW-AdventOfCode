...
for map in maps:
    ...
    for set in map:
        map_v2.append([set[1], set[1] + set[2] - 1, set[0], set[0] + set[2] - 1, set[2], set[0] - set[1]])    #sourceMin, sourceMax, destMin, destMax, range, diff
    
    maps_v2.append(map_v2)
...
while len(maps_v2) > 0:
    ...
    for mapping in last_mappings:
        ...
        for set in maps_v2[0]:
            if mapping >= set[0] and mapping <= set[1]:
                detected_match = True
                tracked_mappings[tracked_mappings_length].append(mapping + set[5])
                break
