...
def calculate_energised_locations(start_location, direction):
    energised_locations.clear()
    save_energised_locations(start_location, direction)
    return len(set([tuple(x) for x in [y[0] for y in energised_locations]]))
   
answers = []
for i in range(col_count):
    answers.append(calculate_energised_locations([0, i], 'S'))
    answers.append(calculate_energised_locations([row_count - 1, i], 'N'))
for i in range(row_count):
    answers.append(calculate_energised_locations([i, 0], 'E'))
    answers.append(calculate_energised_locations([i, col_count - 1], 'W'))
