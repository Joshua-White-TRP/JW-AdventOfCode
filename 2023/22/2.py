...
ans = 0
for support in [x for x in support_map if len(support_map[x]) > 0]:
    q = [support]
    destroyed_bricks = []
    
    while q:
        s = q.pop(0)
        destroyed_bricks.append(s)
        for b in [b for b in support_map[s] if len([x for x in inverse_support_map[b] if x not in destroyed_bricks]) == 0]:
            ans += 1
            q.append(b)
print(ans)